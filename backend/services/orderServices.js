...
export const addNewOrder = async ({userId, amount, firstName, lastName, email, street, city, state, zipCode, country, phone, status }) => {
    const [newOrder] = await db.insert(ordersTable).values({userId, amount, firstName, lastName, email, street, city, state, zipCode, country, phone, status }).$returningId();
    return newOrder.id;
}

export const addNewOrderItems = async (orderItems) => {
    const [orderItem] = await db.insert(orderItemsTable).values(orderItems).$returningId();
    return orderItem.id;
}
